...
from src.navigationUtils import png_to_ogm


class OccupancyGridMap:
    def __init__(self, data_array, cell_size, occupancy_threshold=0, free_threshold=1):
        """
        Creates a grid map
        :param data_array: a 2D array with a value of occupancy per cell (values from 0 - 1)
        :param cell_size: cell size in meters
        :param occupancy_threshold: A threshold to determine whether a cell is occupied.
        :param free_threshold: A threshold to determine whether a cell is free.
        """
        ...
        self.get_all_nodes = self.get_all_nodes()
    ...
    def is_occupied_idx(self, point_idx):
        """
        Check whether the given point is occupied according to the occupancy threshold.
        :param point_idx: a point (x, y) in data array
    ...
    def is_node_idx(self, point_idx):
        """
        Check whether the given point is a node according to the threshold values.
        :param point_idx: a point (x, y) in data array
        :return: True if the given point is a node, false otherwise
        """
        x_index, y_index = point_idx
        if (self.get_data_idx((x_index, y_index)) > self.occupancy_threshold) and \
           (self.get_data_idx((x_index, y_index)) < self.free_threshold):
            return True
        else:
            return False
    ...
    def is_node(self, point):
        ...
        return self.is_node_idx((x_index, y_index))

    def get_all_nodes(self):
        # Search for all nodes
        nodes, nodeslist = [], []
        for j in reversed(range(self.dim_cells[0])):
            for i in range(self.dim_cells[1]):
                pos = self.get_index_from_coordinates(i, j)
                if self.is_node_idx(pos):
                    nodes.append([i, j])
        return nodes
    ...
    def from_png(filename, cell_size):
        ...
        """
        ogm_data_arr = png_to_ogm(filename, normalized=True)
